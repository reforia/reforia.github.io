...
tldr: >-
  Scorpio is a solo-built, RPG-focused framework for Unreal Engine 5 that prioritizes
  architecture over quick-hack tutorials, packaging modular systems—auto save/load world
  state, a message bus, FSM-based quest and dialogue, GAS-integrated skills, and inventory—for
  plug-and-play reuse. Perforce stream mapping lets game-agnostic fixes flow back to a shared
  plugin stream, benefiting all dependent projects.
