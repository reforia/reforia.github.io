...
## Introduction
Scorpio is a RPG kit for Unreal Engine 5. Major systems and architecture was developed by me solo, and for quest and dialogue system, it's a custom adapt for plugin [Narrative 3] (It's not a dependent on `Narrative 3` plugin, I bought the plugin and manually merged the source code into my architecture with changes). For this reason, it's not an open-source project due to licensing, it's a personal project that I want to create as a centralized game framework solution for my other game projects.
...
### Animation
Scorpio will not provide any animation assets or animation blueprints, it's very project specific and it's hard to create a universal solution for it. Plus Unreal Engine already has a very powerful animation system built-in.
