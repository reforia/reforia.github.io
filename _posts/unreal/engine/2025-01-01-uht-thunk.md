...
Before `main()`, the `AutoRegisteredFoo` object will be created since it's a static object, hence calling constructor, which calls `RegisterClass()`, by doing so, we can let `StaticClassFoo` to handle registration automatically on it's own.
