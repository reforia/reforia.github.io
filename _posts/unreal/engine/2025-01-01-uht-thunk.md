---
layout: post
title: "BlueprintNativeEvent Deep Dive. The Magic of UHT"
description:
...
## What and why
We inspected the `UHT` generated file, and we know that the whole purpose of this huge file is to generate the boilerplate code for us, but let's face the elephant in the room:
...
- What would be a good time to register the class information?

Since static object are initialized before `main()` function, it's not a crazy idea to prepare the class information before `main()` function is called. To achieve this, we need to create a static object, and in their constructor, it can call corresponding functions to initialize it, and register information of the class type to somewhere.
...
So what did the `GENERATED_BODY` macro say again? It get's replaced to `{CURRENT_FILE_ID}_{__LINE__}_GENERATED_BODY` in the actual header file. And then `UHT` generates it's definition in the `UHTTest.generated.h` file, which is included in the `UHTTest.h`, effectively replaced `GENERATED_BODY`.
...
### StaticClass()
It's not possible to go through all the details of unreal reflection system in this post. But we can still have a basic understanding of the idea behind.

>A more complete breakdown of the architecture can be found in this post [InsideUE: Type System Code Generation]
{: .prompt-info }

Going through the expanded code, we can see there are a few functions concerning `StaticClass`. (The following code has been re-ordered and simplified for readability), specifically:
- `GetPrivateStaticClass()`
- `StaticClass()` that calls `GetPrivateStaticClass()`
- operator `new` that calls `StaticAllocateObject()` with `StaticClass()`
...
Obviously, the `GetPrivateStaticClass()` is the core function here. From the definition, we can see that when expanding `IMPLEMENT_CLASS_NO_AUTO_REGISTRATION(TClass)` (This is generated into `gen.cpp`) it's does the following:
- Create a `FClassRegistrationInfo` object for the class
- Define a function `GetPrivateStaticClass()` that returns the class's `UClass` object, which is a singleton object that represents the class at runtime, it will be lazily created if it doesn't exist yet.
...
Step into `GetPrivateStaticClassBody()`, we can see that it allocate memory for a `UClass` object, and stores the memory address back to the `ReturnClass` (since it's a reference type`UClass*&`), and then construct it with the necessary information with `placement new` at the `ReturnClass` address. 
...
Next is a bunch of implementations, they are all belonging to other macros that will eventually be expanded to the actual function definitions. They are there for reflection system to retrieve the class information. We will briefly go through them, and talk about the "Why" in the end.
...
Lastly we have the registration part, we have a struct that contains the class information, (BTW, the `3405001915U` and `1070479904` is a class type hash and a declaration hash), and the `Z_Registration_Info_UClass_AUHTTest` holds the class information that will be registered. The most important part is actually the last line `static FRegisterCompiledInInfo`, this actually creates a static `FRegisterCompiledInInfo` object.
