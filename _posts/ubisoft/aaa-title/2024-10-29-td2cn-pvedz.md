...
tldr: >-
  The Division 2's China version reworked the Dark Zone from a 12-player PvPvE
  mode into a 32-player PvE event that rotates through three zones every few
  hours, running 90-minute grinding-boss-extraction rounds for contaminated
  currency. Scaling to 32 players drove server-authoritative gameplay
  programming, network-event batching, entity-filter and GPU-budget
  optimization, and JSON-based Live Config overrides for player caps and session
  timing.
