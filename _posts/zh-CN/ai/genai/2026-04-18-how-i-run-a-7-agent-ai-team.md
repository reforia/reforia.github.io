...
![智能体设置 — "灵魂"存储在数据库中，而非进程里](agent-settings-general.png)
_每个智能体的身份 — 角色、运行时、模型、允许的工具 — 都在 UI 中配置并持久化到 SQLite。_
...
![侧边栏显示全部 7 个智能体及其状态和通知徽章](agent-sidebar.png){: width="250"}
_智能体侧边栏 — 每个智能体显示其最新活动和未读消息数。_
...
![@ 提及自动补全下拉菜单](mention-autocomplete.png){: width="350"}
_输入 `@` 弹出智能体自动补全 — 平台处理路由和上下文注入。_
...
![所有智能体定时任务的月视图日历](task-calendar.png)
_任务日历 — 所有智能体的 cron 任务一目了然。_
...
![使用面板：30 天内 527 次执行、250 万 Token、$387](usage-dashboard.png)
_来自实际部署的真实使用数据 — 7 个智能体，30 天。_
...
![智能体记忆编辑器，支持搜索](memory-editor.png){: width="450"}
_持久化记忆 — 智能体积累的知识条目在会话间保持不变。_
...
![设置向导 — 账户创建步骤](setup-wizard.png){: width="500"}
_设置向导引导你完成账户创建、运行时检测和首个智能体的设置。_
