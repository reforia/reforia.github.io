...
tldr: >-
  Unreal Header Tool 解析 UCLASS 头文件，将反射样板代码——StaticClass 注册、
  GENERATED_BODY 宏以及每个 UFUNCTION 对应的函数跳板（execFoo）——生成到
  .generated.h 和 .gen.cpp 中。对于 BlueprintNativeEvent，UHT 会生成一个经由
  ProcessEvent 和类的 FuncMap 分发的 Foo()，从而让蓝图重写优先解析，仅当不存在
  重写时才回退到 C++ 的 Foo_Implementation。
